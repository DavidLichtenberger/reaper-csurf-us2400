...
if (RPR_CountSelectedTracks(0) > 0) :
	...
	mf_tk = RPR_GetSelectedTrack(0, 0)

	RPR_SetOnlyTrackSelected(mf_tk)
	RPR_Main_OnCommand(RPR_NamedCommandLookup("_SWS_DELALLITEMS"), 0)

	# duplicate mf_tk -> add_tks ('hf'-tk)
	RPR_Main_OnCommand(40062, 0)

	hf_tk = RPR_GetSelectedTrack(0, 0)

	# send main -> add_tks
	Track_CreateSend(main, mf_tk, 2, 0, True, 3)
	Track_CreateSend(main, hf_tk, 4, 0, True, 3)
	...
	RPR_Undo_EndBlock("Split first selected track LMH", 0)