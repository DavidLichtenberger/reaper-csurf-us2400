...
if (RPR_CountSelectedTracks(0) > 0) :
	...
	# insert split tks
	mftk_idx = main_idx + 1
	RPR_InsertTrackAtIndex(mftk_idx, True) 
	RPR_TrackList_AdjustWindows(False)
	...
	mftk = RPR_GetTrack(0, mftk_idx)
	hftk = RPR_GetTrack(0, hftk_idx)

	# name split tks
	Track_Rename(mftk, "< MF", 0)
	Track_Rename(hftk, "< HF", 0)

	# set track channel amount to 10
	Track_SetChannelAmount(mftk, 10, False)
	Track_SetChannelAmount(hftk, 10, False)

	# send main -> split tks
	Track_CreateSend(main, mftk, 0, 0, True, 1)
	Track_CreateSend(main, hftk, 0, 0, True, 1)

	# create sends split tks to folder - 1/2 > 1/2 post (audio) , 9 > 9 pre, 10 > 10 post (autovol)
	Track_CreateSend(mftk, folder, 0, 0, True, 0)
	Track_CreateSend(mftk, folder, 9, 9, False, 0)
	Track_CreateSend(mftk, folder, 8, 8, False, 3)
	...
	RPR_SetTrackSelected(mftk, True)
