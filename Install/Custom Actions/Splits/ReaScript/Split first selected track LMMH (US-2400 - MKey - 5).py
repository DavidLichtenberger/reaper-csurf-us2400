...
def Track_ResetVolPanPhase(tk) :
	...
	chunk = Chunk_ChangeLine("IPHASE", 0, "0", chunk)
...
if (RPR_CountSelectedTracks(0) > 0) :
	...
	# insert split tks
	lmtk_idx = main_idx + 1
	RPR_InsertTrackAtIndex(lmtk_idx, True) 
	RPR_TrackList_AdjustWindows(False)

	hmtk_idx = lmtk_idx + 1
	RPR_InsertTrackAtIndex(hmtk_idx, True) 
	RPR_TrackList_AdjustWindows(False)
	
	hftk_idx = hmtk_idx + 1
	RPR_InsertTrackAtIndex(hftk_idx, True) 
	RPR_TrackList_AdjustWindows(False)
	
	lmtk = RPR_GetTrack(0, lmtk_idx)
	hmtk = RPR_GetTrack(0, hmtk_idx)
	hftk = RPR_GetTrack(0, hftk_idx)

	# name split tks
	Track_Rename(lmtk, "< LM", 0)
	Track_Rename(hmtk, "< HM", 0)
	Track_Rename(hftk, "< HF", 0)

	# set track channel amount to 10
	Track_SetChannelAmount(lmtk, 10, False)
	Track_SetChannelAmount(hmtk, 10, False)
	Track_SetChannelAmount(hftk, 10, False)

	# send main -> split tks
	Track_CreateSend(main, lmtk, 0, 0, True, 1)
	Track_CreateSend(main, hmtk, 0, 0, True, 1)
	Track_CreateSend(main, hftk, 0, 0, True, 1)

	# create sends split tks to folder - 1/2 > 1/2 post (audio) , 9 > 9 pre, 10 > 10 post (autovol)
	Track_CreateSend(lmtk, folder, 0, 0, True, 0)
	Track_CreateSend(lmtk, folder, 9, 9, False, 0)
	Track_CreateSend(lmtk, folder, 8, 8, False, 3)

	Track_CreateSend(hmtk, folder, 0, 0, True, 0)
	Track_CreateSend(hmtk, folder, 9, 9, False, 0)
	Track_CreateSend(hmtk, folder, 8, 8, False, 3)

	Track_CreateSend(hftk, folder, 0, 0, True, 0)
	Track_CreateSend(hftk, folder, 9, 9, False, 0)
	Track_CreateSend(hftk, folder, 8, 8, False, 3)

	# insert split
	Track_InsertFX(main, "(LMMH-Split)")

	# insert autovol
	Track_InsertFX(folder, "(Auto Vol - Split Receive)")
	Track_InsertFX(main, "(Auto Vol - Send)")
	Track_InsertFX(lmtk, "(Auto Vol - Send)")
	Track_InsertFX(hmtk, "(Auto Vol - Send)")
	Track_InsertFX(hftk, "(Auto Vol - Send)")
	...
	RPR_SetTrackSelected(lmtk, True)
	RPR_SetTrackSelected(hmtk, True)
	RPR_SetTrackSelected(hftk, True)
