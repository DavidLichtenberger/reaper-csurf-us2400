...
if (RPR_CountSelectedTracks(0) > 0) :
	...
	# duplicate main -> add_tks ('mf'-tk)and remove items
	RPR_SetOnlyTrackSelected(main)
	RPR_Main_OnCommand(40062, 0)

	lm_tk = RPR_GetSelectedTrack(0, 0)

	RPR_SetOnlyTrackSelected(lm_tk)
	RPR_Main_OnCommand(RPR_NamedCommandLookup("_SWS_DELALLITEMS"), 0)

	# duplicate lm_tk -> add_tks ('hm'-tk)
	RPR_Main_OnCommand(40062, 0)

	hm_tk = RPR_GetSelectedTrack(0, 0)

	# duplicate hm_tk -> add_tks ('hf'-tk)
	RPR_SetOnlyTrackSelected(hm_tk)
	RPR_Main_OnCommand(40062, 0)

	hf_tk = RPR_GetSelectedTrack(0, 0)

	# send main -> add_tks
	Track_CreateSend(main, lm_tk, 2, 0, True, 3)
	Track_CreateSend(main, hm_tk, 4, 0, True, 3)
	Track_CreateSend(main, hf_tk, 6, 0, True, 3)
	...
	Track_InsertFX(lm_tk, "(Auto Vol - Send)")
	Track_InsertFX(hm_tk, "(Auto Vol - Send)")
	Track_InsertFX(hf_tk, "(Auto Vol - Send)")

	# rename tracks
	Track_Rename(folder, " (Split)", 1)
	Track_Rename(main, "< LF", 0)
	Track_Rename(lm_tk, "< LM", 0)
	Track_Rename(hm_tk, "< HM", 0)
	Track_Rename(hf_tk, "< HF", 0)

	# select all tracks
	RPR_SetOnlyTrackSelected(folder)
	RPR_SetTrackSelected(main, True)
	RPR_SetTrackSelected(lm_tk, True)
	RPR_SetTrackSelected(hm_tk, True)
	RPR_SetTrackSelected(hf_tk, True)
	...
	RPR_Undo_EndBlock("Split first selected track LMMH", 0)