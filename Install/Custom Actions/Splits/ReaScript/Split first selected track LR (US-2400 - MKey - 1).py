...
if (RPR_CountSelectedTracks(0) > 0) :
	...
	# duplicate main -> add_tks ('r'-tk)and remove items
	RPR_SetOnlyTrackSelected(main)
	RPR_Main_OnCommand(40062, 0)

	r_tk = RPR_GetSelectedTrack(0, 0)

	RPR_SetOnlyTrackSelected(r_tk)
	RPR_Main_OnCommand(RPR_NamedCommandLookup("_SWS_DELALLITEMS"), 0)

	# width and pan
	RPR_SetMediaTrackInfo_Value(main, "D_WIDTH", 0.0);
	RPR_SetMediaTrackInfo_Value(main, "D_PAN", -1.0);

	RPR_SetMediaTrackInfo_Value(r_tk, "D_WIDTH", 0.0);
	RPR_SetMediaTrackInfo_Value(r_tk, "D_PAN", 1.0);

	# send main -> add_tks
	Track_CreateSend(main, r_tk, 2, 0, True, 3)
	...
	Track_InsertFX(r_tk, "(Auto Vol - Send)")

	# rename tracks
	Track_Rename(folder, " (Split)", 1)
	Track_Rename(main, "< L", 0)
	Track_Rename(r_tk, "< R", 0)

	# select all tracks
	RPR_SetOnlyTrackSelected(folder)
	RPR_SetTrackSelected(main, True)
	RPR_SetTrackSelected(r_tk, True)
	...
	RPR_Undo_EndBlock("Split first selected track LR", 0)