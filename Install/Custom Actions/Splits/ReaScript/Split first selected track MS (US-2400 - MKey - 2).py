...
if (RPR_CountSelectedTracks(0) > 0) :
	...
	# insert split tks
	stk_idx = main_idx + 1
	RPR_InsertTrackAtIndex(stk_idx, True) 
	RPR_TrackList_AdjustWindows(False)
	stk = RPR_GetTrack(0, stk_idx)

	# name split tks
	Track_Rename(stk, "< S", 0)

	# set track channel amount to 10
	Track_SetChannelAmount(stk, 10, False)

	# send main -> split tks
	Track_CreateSend(main, stk, 0, 0, True, 1)

	# create sends split tks to folder - 1/2 > 1/2 post (audio) , 9 > 9 pre, 10 > 10 post (autovol)
	Track_CreateSend(stk, folder, 0, 0, True, 0)
	Track_CreateSend(stk, folder, 9, 9, False, 0)
	Track_CreateSend(stk, folder, 8, 8, False, 3)

	# insert split
	Track_InsertFX(main, "(MS-Split)")

	# insert autovol
	Track_InsertFX(folder, "(Auto Vol - Add Receive)")
	Track_InsertFX(main, "(Auto Vol - Send)")
	Track_InsertFX(stk, "(Auto Vol - Send)")
