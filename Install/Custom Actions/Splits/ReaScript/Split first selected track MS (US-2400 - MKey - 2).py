...
if (RPR_CountSelectedTracks(0) > 0) :
	...
	# duplicate main -> add_tks ('s'-tk)and remove items
	RPR_SetOnlyTrackSelected(main)
	RPR_Main_OnCommand(40062, 0)

	s_tk = RPR_GetSelectedTrack(0, 0)

	RPR_SetOnlyTrackSelected(s_tk)
	RPR_Main_OnCommand(RPR_NamedCommandLookup("_SWS_DELALLITEMS"), 0)

	# send main -> add_tks
	Track_CreateSend(main, s_tk, 2, 0, True, 3)
	...
	Track_InsertFX(s_tk, "(Auto Vol - Send)")

	# rename tracks
	Track_Rename(folder, " (Split)", 1)
	Track_Rename(main, "< M", 0)
	Track_Rename(s_tk, "< S", 0)

	# select all tracks
	RPR_SetOnlyTrackSelected(folder)
	RPR_SetTrackSelected(main, True)
	RPR_SetTrackSelected(s_tk, True)
	...
	RPR_Undo_EndBlock("Split first selected track MS", 0)