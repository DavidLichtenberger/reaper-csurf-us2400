...
if (RPR_CountSelectedTracks(0) > 0) :
	...
	# rename tracks
	Track_Rename(folder, main_name + " (PP)", 0)
	...
	# insert split tks
	pptk_idx = main_idx + 1
	RPR_InsertTrackAtIndex(pptk_idx, True) 
	RPR_TrackList_AdjustWindows(False)
	pptk = RPR_GetTrack(0, pptk_idx)

	# name split tks
	Track_Rename(pptk, "< PP", 0)

	# set track channel amount to 10
	Track_SetChannelAmount(pptk, 10, False)

	# send main -> split tks
	Track_CreateSend(main, pptk, 0, 0, True, 1)

	# create sends split tks to folder - 1/2 > 1/2 post (audio) , 9 > 9 pre, 10 > 10 post (autovol)
	Track_CreateSend(pptk, folder, 0, 0, True, 0)
	Track_CreateSend(pptk, folder, 9, 9, False, 0)
	Track_CreateSend(pptk, folder, 8, 8, False, 3)

	# insert autovol
	Track_InsertFX(folder, "(Auto Vol - Add Receive)")
	Track_InsertFX(main, "(Auto Vol - Send)")
	Track_InsertFX(pptk, "(Auto Vol - Send)")
	...
	RPR_SetTrackSelected(pptk, True)
